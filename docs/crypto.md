...
**Signing (custom code, ~100 lines BigInt):** The X25519 private scalar (clamped) is used for Ed25519-style scalar multiplication on the Edwards curve. If the resulting public key has odd x-coordinate, the scalar is negated mod L to force even x (sign bit 0). Deterministic nonce via `SHA-512(0xFE*32 || scalar || message)` (same domain separator as the XEdDSA spec). The signing path uses custom BigInt arithmetic that has not been independently audited. It is NOT constant-time (acceptable in this threat model — see comment in `src/sign.ts`). Correctness is validated by deterministic test vectors and cross-verification against Web Crypto `Ed25519.verify()`. This is a custom construction for this application only — do not treat it as a generic XEdDSA interoperability guarantee.
